...
```powershell
cd "[Your_Work_Directory]\DataAnalysis\learning\courses\ibm\dataanalyst\08-data-visualization-with-python\activities\hands-on\09-dash-basics-html-core-components"
```
...
```text
...\09-dash-basics-html-core-components\.venv\Scripts\python.exe
```
...
> **Note:** When running a Python file directly, use `python dash_basics.py`.
>
> If using the module syntax, use `python -m dash_basics` — **do not include `.py`**.
